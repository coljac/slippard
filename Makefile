...
build_macos_amd64:
	mkdir -p $(MACOS_DIR)
	GOOS=darwin GOARCH=amd64 go build -o $(MACOS_DIR)/$(APP_NAME) ./cmd/slpd
	tar -czvf $(MACOS_DIR)/$(APP_NAME)_$(VERSION)_macos_amd64.tar.gz -C $(MACOS_DIR) $(APP_NAME)

build_macos_arm64:
	mkdir -p $(MACOS_DIR)
	GOOS=darwin GOARCH=arm64 go build -o $(MACOS_DIR)/$(APP_NAME) ./cmd/slpd
	tar -czvf $(MACOS_DIR)/$(APP_NAME)_$(VERSION)_macos_arm64.tar.gz -C $(MACOS_DIR) $(APP_NAME)

build_linux_amd64:
	mkdir -p $(LINUX_AMD64_DIR)
	GOOS=linux GOARCH=amd64 go build -o $(LINUX_AMD64_DIR)/$(APP_NAME) ./cmd/slpd
	tar -czvf $(LINUX_AMD64_DIR)/$(APP_NAME)_$(VERSION)_linux_amd64.tar.gz -C $(LINUX_AMD64_DIR) $(APP_NAME)

build_linux_arm64:
	mkdir -p $(LINUX_ARM64_DIR)
	GOOS=linux GOARCH=arm64 go build -o $(LINUX_ARM64_DIR)/$(APP_NAME) ./cmd/slpd
	tar -czvf $(LINUX_ARM64_DIR)/$(APP_NAME)_$(VERSION)_linux_arm64.tar.gz -C $(LINUX_ARM64_DIR) $(APP_NAME)

build_windows:
	mkdir -p $(WINDOWS_DIR)
	GOOS=windows GOARCH=amd64 go build -o $(WINDOWS_DIR)/$(APP_NAME).exe ./cmd/slpd
